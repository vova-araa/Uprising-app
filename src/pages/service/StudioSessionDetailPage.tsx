    ...
    <div className="min-h-full pb-24">
      ...
      <div className="px-5 pt-4 pb-6">
        ...
        <div className="flex items-center gap-4">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl icon-tile">
            <Mic size={28} className="text-white" strokeWidth={2.1} />
          </div>
          <div>
            <h1 className="text-2xl font-extrabold font-display tracking-[-0.01em]">{t("studioSessionTitle")}</h1>
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="px-5 space-y-6">
        ...
        <motion.div variants={item}>
          <div className="flex items-center gap-2 mb-3 px-1">
            <span className="h-5 w-1 rounded-full accent-bar" />
            <h3 className="font-extrabold font-display text-sm tracking-[-0.01em]">{t("suitableFor")}</h3>
          </div>
          <div className="grid grid-cols-2 gap-2">
            {sessionTypes.map((st, i) => (
              <div
                key={st.nl}
                className="relative rounded-2xl card-feature border border-white/5 p-4 overflow-hidden aspect-[5/4] flex flex-col justify-between hover:-translate-y-0.5 transition-all"
              >
